...
- **FEITO em 11/06, no ar e TESTADO ✅ (código OK):** **seletor de ESTILO + geração de imagem com Gemini (nano banana)**. O seletor de capa virou escolha de **estilo** (não foto fixa); ao "Gerar artes", se a peça tem estilo escolhido, a IA gera uma **imagem de fundo nova** nesse estilo (`src/lib/social/imagem.ts`, modelo `gemini-2.5-flash-image`, `GEMINI_API_KEY` no Vercel). Tinta e "Subir foto" seguem como antes. **Teste ao vivo (11/06):** a chamada autenticou e o request está correto — só voltou **`429 quota exceeded`**. **➜ FALTA SÓ ATIVAR BILLING** no Google: o nível grátis não cobre geração de imagem. Vá em **https://aistudio.google.com → Get API key / Billing** (ou Google Cloud Console → o projeto da chave → Billing → vincular conta de cobrança) e ative. Depois "Gerar artes" com estilo passa a funcionar (~$0.039/imagem). NENHUMA mudança de código necessária.
- **PENDENTE (próximos passos):**
  1. **Ativar billing do Gemini** (acima) pra liberar a geração de imagem. Sem código.
