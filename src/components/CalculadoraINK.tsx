...
function RadarINK({ r, c, p }: { r: number; c: number; p: number }) {
  ...
  const rotulos = [
    { letra: 'R', nome: 'Reconhecimento', valor: r },
    { letra: 'C', nome: 'Coerência', valor: c },
    { letra: 'P', nome: 'Permanência', valor: p },
  ];
  return (
...
export default function CalculadoraINK() {
  ...
  const getEstagio = (valor: number) => {
    if (valor <= 30) return {
      nome: 'NARRATIVA AUSENTE', bg: 'bg-black', text: 'text-white',
    ...
    if (valor <= 60) return {
      nome: 'EM TRANSIÇÃO', bg: 'bg-neutral-700', text: 'text-white',
    ...
    if (valor <= 80) return {
      nome: 'VIROU ATIVO', bg: 'bg-neutral-300', text: 'text-black',
    ...
    return {
      nome: 'ATIVO PLENO', bg: 'bg-white border-2 border-black', text: 'text-black',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitError('');
    if (!nome.trim()) { setSubmitError('Por favor, informe seu nome.'); return; }
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!emailRegex.test(email)) { setSubmitError('Por favor, informe um e-mail válido.'); return; }
    if (!consentimento) { setSubmitError('É necessário concordar com a Política de Privacidade.'); return; }
    setSubmitting(true);
    try {
      ...
      setSubmitError('Houve um erro ao enviar. Tente novamente em instantes.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const baixarPDF = () => {
    ...
    doc.setFont('times', 'bold'); doc.setFontSize(22); doc.text('Calculadora INK', margin, y); y += 7;
    doc.setFont('times', 'italic'); doc.setFontSize(12); doc.setTextColor(80); doc.text('Índice de Narrativa em KPI', margin, y); y += 5;
    doc.setFont('times', 'normal'); doc.setFontSize(9); doc.setTextColor(120); doc.text('Bruno Massa  |  Narrativa em KPI', margin, y); y += 8;
    ...
    doc.text(`Diagnóstico gerado em ${dataFormatada}`, margin, y); y += 10;

    doc.setTextColor(40); doc.setFont('times', 'bold'); doc.setFontSize(11); doc.text('OS TRÊS COMPONENTES', margin, y); y += 6;
    const componentes = [
      { letra: 'R', nome: 'Reconhecimento prévio', valor: r },
      { letra: 'C', nome: 'Coerência percebida', valor: c },
      { letra: 'P', nome: 'Permanência espontânea', valor: p },
    ];
    ...
    doc.setFont('times', 'normal'); doc.setFontSize(10); doc.setTextColor(100); doc.text('SEU INK', pageWidth / 2, y, { align: 'center' }); y += 12;
    doc.setFont('times', 'bold'); doc.setFontSize(54); doc.setTextColor(0); doc.text(`${ink}`, pageWidth / 2, y, { align: 'center' }); y += 6;
    doc.setFont('times', 'italic'); doc.setFontSize(9); doc.setTextColor(120); doc.text('INK = ( R + C + P ) ÷ 3', pageWidth / 2, y, { align: 'center' }); y += 12;
    doc.setDrawColor(0); doc.line(margin, y, pageWidth - margin, y); y += 10;

    doc.setFont('times', 'normal'); doc.setFontSize(10); doc.setTextColor(100); doc.text('ESTÁGIO', margin, y); y += 7;
    ...
    doc.setDrawColor(180); doc.setLineWidth(0.2); doc.line(margin, y, pageWidth - margin, y); y += 8;
    doc.setFont('times', 'bold'); doc.setFontSize(11); doc.setTextColor(40); doc.text('MAPA DA NARRATIVA', margin, y); y += 4;
    ...
    if (showDerivadas) {
      ...
      doc.setDrawColor(180); doc.setLineWidth(0.2); doc.line(margin, y, pageWidth - margin, y); y += 8;
      doc.setFont('times', 'bold'); doc.setFontSize(11); doc.setTextColor(40); doc.text('INDICADORES DERIVADOS', margin, y); y += 8;
      doc.setFont('times', 'normal'); doc.setFontSize(10);
      doc.text(`CAC efetivo informado: ${formatBRL(cac)}`, margin, y); y += 5;
      doc.text(`Investimento total informado: ${formatBRL(investimento)}`, margin, y); y += 8;
      doc.setFont('times', 'bold'); doc.setFontSize(10); doc.setTextColor(100); doc.text('CAC NARRATIVO', margin, y); y += 6;
      doc.setFontSize(18); doc.setTextColor(0); doc.text(formatBRL(cacNarrativo), margin, y); y += 6;
      doc.setFont('times', 'italic'); doc.setFontSize(9); doc.setTextColor(100); doc.text('parcela do CAC paga para compensar coerência ausente', margin, y); y += 8;
      doc.setFont('times', 'bold'); doc.setFontSize(10); doc.text('CUSTO DE FRICÇÃO', margin, y); y += 6;
      doc.setFontSize(18); doc.setTextColor(0); doc.text(formatBRL(custoFriccao), margin, y); y += 6;
      doc.setFont('times', 'italic'); doc.setFontSize(9); doc.setTextColor(100); doc.text('verba gasta para tapar buraco de coerência', margin, y); y += 10;
    }
    ...
    doc.text('Instrumento autoral do livro "Narrativa em KPI", de Bruno Massa.', pageWidth / 2, rodapeY + 5, { align: 'center' });
    doc.text('brunomassa.online/calculadora', pageWidth / 2, rodapeY + 10, { align: 'center' });
    doc.save(`Diagnostico-INK-${new Date().toISOString().split('T')[0]}.pdf`);
  };
    ...
    <div className="min-h-screen bg-stone-50 py-8 px-4 font-serif">
      <div className="max-w-3xl mx-auto bg-white shadow-sm border border-neutral-200 p-8 md:p-12">
        <header className="border-b border-neutral-300 pb-6 mb-8">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-black">Calculadora INK</h1>
          <p className="text-base text-neutral-600 mt-1 italic">Índice de Narrativa em KPI</p>
        ...
        <div className="mb-8 text-sm text-neutral-700 leading-relaxed">
          <p>Estime os três componentes da narrativa da sua empresa em escala de zero a cem. O INK é a média simples dos três, na mesma escala. Não precisa ser preciso. Precisa ser honesto.</p>
        </div>

        <div className="space-y-7 mb-10">
          <div>
            <div className="flex items-baseline justify-between mb-1">
              <label className="font-bold text-base"><span className="text-xl">R</span> &nbsp; Reconhecimento prévio</label>
              <span className="font-bold text-2xl tabular-nums">{r}</span>
            </div>
            <p className="text-xs text-neutral-600 italic mb-2">% de novos clientes que chegam à empresa já reconhecendo a marca</p>
          ...
          <div>
            <div className="flex items-baseline justify-between mb-1">
              <label className="font-bold text-base"><span className="text-xl">C</span> &nbsp; Coerência percebida</label>
              <span className="font-bold text-2xl tabular-nums">{c}</span>
            </div>
            <p className="text-xs text-neutral-600 italic mb-2">% de clientes que descrevem a empresa de forma convergente em pesquisa</p>
          ...
          <div>
            <div className="flex items-baseline justify-between mb-1">
              <label className="font-bold text-base"><span className="text-xl">P</span> &nbsp; Permanência espontânea</label>
              <span className="font-bold text-2xl tabular-nums">{p}</span>
            </div>
            <p className="text-xs text-neutral-600 italic mb-2">% de retenção orgânica, sem programa de fidelidade pesado</p>
        ...
        <div className="border-t border-b border-black py-8 mb-8 text-center relative">
          <p className="text-xs uppercase tracking-widest text-neutral-600 mb-2">Seu INK</p>
          {unlocked ? (
            <div className="fade-in-ink">
              <p className="text-7xl md:text-8xl font-bold tabular-nums leading-none">{ink}</p>
              <p className="text-xs text-neutral-500 mt-2 italic">INK = ( R + C + P ) ÷ 3</p>
            </div>
          ) : (
            <div className="relative">
              ...
              <p className="text-xs text-neutral-500 mt-2 italic select-none" style={{ filter: 'blur(4px)' }}>INK = ( R + C + P ) ÷ 3</p>
              <div className="absolute inset-0 flex items-center justify-center">
                <button onClick={() => setShowModal(true)} className="bg-black text-white px-6 py-3 text-sm font-bold tracking-wide hover:bg-neutral-800 transition-colors shadow-lg">REVELAR MEU RESULTADO →</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mb-8">
          <p className="text-xs uppercase tracking-widest text-neutral-600 mb-3 text-center">A Régua do INK</p>
          <div className="relative h-16 flex border border-neutral-400">
            <div className="bg-black flex items-center justify-center" style={{ width: '30%' }}><span className="text-[10px] text-white font-bold tracking-wide">AUSENTE</span></div>
            <div className="bg-neutral-700 flex items-center justify-center" style={{ width: '30%' }}><span className="text-[10px] text-white font-bold tracking-wide">TRANSIÇÃO</span></div>
            <div className="bg-neutral-300 flex items-center justify-center" style={{ width: '20%' }}><span className="text-[10px] text-black font-bold tracking-wide">VIROU ATIVO</span></div>
            <div className="bg-white flex items-center justify-center" style={{ width: '20%' }}><span className="text-[10px] text-black font-bold tracking-wide">PLENO</span></div>
            {unlocked && (
        ...
        {unlocked ? (
          <div className={`${estagio.bg} ${estagio.text} p-6 mb-8 fade-in-ink`}>
            <p className="text-xs uppercase tracking-widest opacity-70 mb-2">Estágio</p>
          ...
          <div className="border-2 border-dashed border-neutral-300 p-6 mb-8 text-center">
            <p className="text-sm text-neutral-600 italic">Diagnóstico completo do seu estágio narrativo disponível após o cadastro acima.</p>
          </div>
        )}

        {unlocked && (
          <div className="mb-8 fade-in-ink">
            <p className="text-xs uppercase tracking-widest text-neutral-600 mb-3 text-center">Mapa da Narrativa</p>
            <RadarINK r={r} c={c} p={p} />
            <p className="text-xs text-neutral-500 italic text-center mt-2">Quanto mais amplo e equilibrado o triângulo, mais a narrativa sustenta a operação.</p>
          </div>
        )}

        {unlocked && (
          <div className="mb-8 text-center fade-in-ink">
            <button onClick={baixarPDF} className="border-2 border-black px-6 py-3 text-sm font-bold tracking-wide hover:bg-black hover:text-white transition-colors">↓ BAIXAR DIAGNÓSTICO EM PDF</button>
          </div>
        )}
        ...
        {unlocked && (
          <div className="border-t border-neutral-300 pt-6 fade-in-ink">
            <button onClick={() => setShowDerivadas(!showDerivadas)} className="flex items-center gap-2 text-sm font-bold text-black hover:text-neutral-700 transition-colors">
              <span className="text-lg leading-none">{showDerivadas ? '−' : '+'}</span>Calcular CAC Narrativo e Custo de Fricção
            </button>
            {showDerivadas && (
              <div className="mt-6 space-y-5">
                <div className="grid md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-xs uppercase tracking-wide text-neutral-700 font-bold mb-1">CAC efetivo (R$)</label>
                    ...
                    <p className="text-xs text-neutral-500 italic mt-1">custo médio para conquistar um novo cliente</p>
                  </div>
                  <div>
                    <label className="block text-xs uppercase tracking-wide text-neutral-700 font-bold mb-1">Investimento total (R$)</label>
                    ...
                    <p className="text-xs text-neutral-500 italic mt-1">verba mensal de aquisição (mídia + comercial)</p>
                  </div>
                </div>
                <div className="grid md:grid-cols-2 gap-5 pt-4">
                  <div className="border border-neutral-300 p-5">
                    <p className="text-xs uppercase tracking-widest text-neutral-600 mb-1">CAC Narrativo</p>
                    <p className="text-3xl font-bold tabular-nums mb-1">{formatBRL(cacNarrativo)}</p>
                    <p className="text-xs text-neutral-600 italic">parcela do CAC paga para compensar coerência ausente</p>
                    <p className="text-[10px] text-neutral-400 mt-2 font-mono">CAC × ( 1 − INK ÷ 100 )</p>
                  </div>
                  <div className="border border-neutral-300 p-5">
                    <p className="text-xs uppercase tracking-widest text-neutral-600 mb-1">Custo de Fricção</p>
                    <p className="text-3xl font-bold tabular-nums mb-1">{formatBRL(custoFriccao)}</p>
                    <p className="text-xs text-neutral-600 italic">verba gasta para tapar buraco de coerência</p>
                    <p className="text-[10px] text-neutral-400 mt-2 font-mono">Investimento × ( 1 − INK ÷ 100 )</p>
                  </div>
                </div>
                <p className="text-xs text-neutral-600 leading-relaxed pt-2 border-t border-neutral-200">Quanto maior o INK, menor o CAC Narrativo e o Custo de Fricção. Subir o INK em vinte pontos significa, para essa operação, reduzir o CAC Narrativo e o Custo de Fricção em proporção direta. Esse é o ganho realocável que sustenta o investimento em articulação narrativa.</p>
              </div>
            )}
          </div>
        )}
        ...
        <footer className="mt-12 pt-6 border-t border-neutral-300 text-center">
          <p className="text-xs text-neutral-500 italic">Instrumento autoral do livro <span className="font-bold not-italic">Narrativa em KPI</span>, de Bruno Massa</p>
          <p className="text-[10px] text-neutral-400 mt-1">Marketing, coerência e o ativo invisível que sustenta empresas duradouras</p>
          <p className="text-[10px] text-neutral-400 mt-2"><a href="/privacidade" className="underline hover:text-neutral-600">Política de Privacidade</a></p>
        </footer>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0, 0, 0, 0.6)' }}>
          <div className="bg-white max-w-md w-full p-8 fade-in-ink">
            ...
            <p className="text-sm text-neutral-600 italic mb-6">Preencha para revelar seu diagnóstico completo.</p>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-xs uppercase tracking-wide font-bold mb-1">Nome</label>
                <input type="text" value={nome} onChange={(e) => setNome(e.target.value)} className="w-full border border-neutral-400 px-3 py-2 text-base focus:outline-none focus:border-black" placeholder="Seu nome" autoFocus />
              </div>
              <div>
                <label className="block text-xs uppercase tracking-wide font-bold mb-1">E-mail</label>
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full border border-neutral-400 px-3 py-2 text-base focus:outline-none focus:border-black" placeholder="seu@email.com" />
              </div>
              <label className="flex items-start gap-2 text-xs text-neutral-700 cursor-pointer">
                ...
                <span>Concordo em receber comunicações sobre Narrativa em KPI e li a <a href="/privacidade" target="_blank" rel="noopener noreferrer" className="underline font-bold">Política de Privacidade</a>.</span>
              </label>
              {submitError && <p className="text-xs text-red-600 font-bold">{submitError}</p>}
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 border border-neutral-400 px-4 py-3 text-sm font-bold hover:bg-neutral-100 transition-colors" disabled={submitting}>Cancelar</button>
                <button type="submit" className="flex-1 bg-black text-white px-4 py-3 text-sm font-bold hover:bg-neutral-800 transition-colors disabled:opacity-50" disabled={submitting}>{submitting ? 'Enviando...' : 'Ver resultado'}</button>
              </div>
              <p className="text-[10px] text-neutral-500 italic pt-2 border-t border-neutral-200">Seus dados são tratados conforme a LGPD. Não compartilhamos com terceiros.</p>
            </form>
          </div>
        </div>
      )}
