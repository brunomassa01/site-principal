...
// (verificação ascii-fold). Processa em lotes (?limite=) pra não estourar o tempo da function.
...
export const POST: APIRoute = async ({ url }) => {
  ...
  // pega peças das semanas 1-52 com gancho OU lente sem acento
  const rows = await db
  ...
  // monta a fila de campos a corrigir
  ...
  let atualizados = 0;
  let pulados = 0;
  const exemplos: { de: string; para: string }[] = [];
  // agrupa updates por peça (gancho/lente podem cair na mesma)
  const patchPorPeca: Record<string, Record<string, string>> = {};
  for (let i = 0; i < lote.length; i++) {
    const it = lote[i];
    const novo = (corrigidos[i] ?? '').trim();
    // segurança: só aceita se, removendo acentos, o texto é idêntico (nada além de acento mudou)
    if (novo && novo !== it.original && fold(novo) === fold(it.original)) {
      (patchPorPeca[it.id] ??= {})[it.campo] = novo;
      if (exemplos.length < 4) exemplos.push({ de: it.original.slice(0, 50), para: novo.slice(0, 50) });
      atualizados++;
    } else {
      pulados++;
    }
